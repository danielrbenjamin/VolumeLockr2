...
class VolumeAdapter(
    ...
    private fun loadLockFromService(holder: ViewHolder, volume: Volume) {
        val locks = mService?.getLocks(volume.deviceType)?.keys
        locks?.let {
            for (key in it) {
    ...
    private fun adjustService(deviceType: String) {
        mService?.getLocks(deviceType)?.let {
            if (it.isNotEmpty()) {
    ...
    private fun handleRingerMode(holder: ViewHolder, volume: Volume) {
        if (volume.stream == AudioManager.STREAM_NOTIFICATION) {
            holder.binding.slider.isEnabled =
                mService?.getMode() == 2 &&
                mService?.getLocks(volume.deviceType)?.containsKey(AudioManager.STREAM_NOTIFICATION) == false
        }
    }

    private fun onVolumeLocked(holder: ViewHolder, volume: Volume) {
        mService?.let {
            it.addLock(volume.stream, volume.value, volume.deviceType)
            VolumeService.start(mContext)
            adjustService(volume.deviceType)
            adjustNotification()
            holder.binding.slider.isEnabled = false
        }
    }
    ...
    private fun onVolumeUnlocked(holder: ViewHolder, volume: Volume) {
        mService?.let {
            it.removeLock(volume.stream, volume.deviceType)
            adjustService(volume.deviceType)
            adjustNotification()
